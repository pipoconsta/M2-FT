...
function buildToDo(todo, index) {
  // Tu código acá:
  let toDoShell = document.createElement("div") // Crear variable para añadir un div

  toDoShell.setAttribute("class","toDoShell") // CREA LA CLASE Y LE ASIGNA "toDoShell"

  let toDoText = document.createElement("span")

  toDoText.innerHTML = todo.description //CORRESPONDE A LA DESCRIPCI+ON DEL DIV CREADO ANTERIORMENTE
  toDoText.setAttribute("id", index)

  toDoText.addEventListener("click", "completeText");

  if(todo.complete) {
    toDoText.setAttribute("class", "completeText")
  }

  toDoShell.appendChild(toDoText) //ASOCIAMOS EL SPAN EN EL DIV
...
function buildToDos(toDos) {
  // Tu código acá:
 let array = toDos.map(buildToDo(todo, index))
 return array;
}
...
function displayToDos() {
  // Tu código acá:
  let toDoContainer = document.getElementById("toDoContainer") // NO HACE FALTA EL # YA QUE BUSCA DIRECTO EL ID
  toDoContainer.innerHTML = ""

  let result = buildToDos(toDoItems)

  result.map(element => toDoContainer.appendChild(element)) 
...
function addToDo() {
  // Tu código acá:
  let description = document.getElementById("toDoInput").value // EN ESTE EXTRAEMOS EL VALOR
  ...
  let inpt = document.getElementById("toDoInput") 
   inpt.value = "" // ACA HACEMOS FOCO DIRECTAMENTE AL INPUT
